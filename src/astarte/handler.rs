...
use super::sdk::DynamicIntrospection;
...
pub fn init_pub_sub<C>(client: C) -> (DevicePublisher<C>, DeviceSubscriber<C>)
where
    C: Client,
{
...
pub struct DeviceSubscriber<C> {
    client: C,
    subscribers: SubscribersMap,
}

impl<C> DeviceSubscriber<C> {
    /// Create a new client.
    pub fn new(client: C, subscribers: SubscribersMap) -> Self {
        Self {
            client,
            subscribers,
        }
    ...
    async fn on_event(&mut self, event: DeviceEvent) -> Result<(), DeviceError> {
...
///
/// Uses the [`Client`] to provide subscribe and publish functionality.
#[derive(Clone)]
pub struct DevicePublisher<C> {
    client: C,
    subscribers: SubscribersMap,
}

impl<C> DevicePublisher<C> {
    /// Constructs a new handler from the [AstarteDeviceSdk](astarte_device_sdk::AstarteDeviceSdk)
    fn new(client: C, subscribers: SubscribersMap) -> Self {
        DevicePublisher {
            client,
            subscribers,
        }
...
#[async_trait]
impl<C> AstarteSubscriber for DevicePublisher<C>
where
    C: Client + DynamicIntrospection + PropAccess + Send + Sync,
{
...
#[async_trait]
impl<C> AstartePublisher for DevicePublisher<C>
where
    C: Client + Send + Sync,
{
    async fn publish(
...
impl<C> PropAccessExt for DevicePublisher<C>
where
    C: PropAccess + Send + Sync,
{
    async fn property(
        &self,
        node_id: NodeId,
...
mod test {
    ...
    use crate::tests::{MockClient, MockConnection};
    ...
    fn mock_with_clone<F>(mut seq: Sequence, client: &mut MockClient, f: F)
    where
        F: FnOnce(Sequence, MockClient) -> MockClient + Send + 'static,
    {
        client
            .expect_clone()
            .once()
            .in_sequence(&mut seq)
            .return_once(move || f(seq, MockClient::new()));
    }
    ...
    async fn publish_individual_failed() {
        ...
        mock_with_clone(seq, &mut client, move |seq, mut client| {
            mock_with_clone(seq, &mut client, move |mut seq, mut client| {
                client
                    ...
                    .returning(|_, _, _| {
                        Err(AstarteError::new(ErrorKind::Interface(
                            astarte_device_sdk::error::InterfaceError::MappingNotFound,
                        )))
                    });
        ...
        let result_err = result.unwrap_err();
        assert!(
            matches!(result_err, AstarteMessageHubError::Astarte(err) if *err.kind() == ErrorKind::Interface(astarte_device_sdk::error::InterfaceError::MappingNotFound))
        );
    }
    ...
    async fn publish_individual_with_timestamp_failed() {
        ...
        assert!(matches!(
            result_err,
            AstarteMessageHubError::Astarte(err) if *err.kind() == ErrorKind::Interface(astarte_device_sdk::error::InterfaceError::MappingNotFound),
        ));
    }
