...
use super::{DEFAULT_GRPC_PORT, DEFAULT_HOST, Legacy, MessageHubOptions, Override, Pairing};
        ...
        let pairing = if let Some(fdo) = value.fdo.take_if(|fdo| fdo.enabled.unwrap_or_default()) {
            cfg_if::cfg_if! {
                if #[cfg(feature = "fdo")] {
                    ...
                    Pairing::Fdo(crate::config::Fdo {
                        manufacturing_url,
                        device_id,
                    })
                } else {
                    tracing::trace!(?fdo, "couldn't use fdo config, compile with fdo feature");

                    return Err(ConfigError::Feature("fdo"))
                }
            }
        } else {
            let device_id = value
