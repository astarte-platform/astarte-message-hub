...
impl HttpConfigProvider {
    pub(self) async fn config(
        Extension(state): Extension<ConfigServerExtension>,
        Json(payload): Json<ConfigPayload>,
    ) -> impl IntoResponse {
        let messagehub_options = MessageHubOptions {
        ...
        let result =
            std::fs::File::create(format!("{}/message-hub-config.toml", state.store_directory));
        if let Err(_) = result {
            return (
        ...
        let result = toml::to_string(&messagehub_options);
        if let Err(_) = result {
            return (
                StatusCode::INTERNAL_SERVER_ERROR,
                Json(ConfigResponse {
