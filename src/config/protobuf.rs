...
pub struct ProtobufConfigProvider {
    ...
    handle: JoinHandle<Result<(), ProtobufServerError>>,
}
...
impl ProtobufConfigProvider {
    ...
    pub async fn new(
        ...
        configuration_ready_channel: Arc<Notify>,
        toml_file: &str,
    ) -> Result<ProtobufConfigProvider, ProtobufServerError> {
        ...
        let handle = tokio::spawn(async move {
            Server::builder()
                ...
                .await
                .map_err(ProtobufServerError::Start)
        });
    ...
    pub async fn stop(self) -> Result<(), ProtobufServerError> {
        self.shutdown_channel.send(()).await?;
        self.handle.await.map_err(ProtobufServerError::Join)?
    }
}
